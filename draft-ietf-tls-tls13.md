...
The pre_master_secret is used to generate a number of secret values,
as detailed below.

First, as soon as the ClientKeyShares and ServerKeyShares messages
have been exchanged, the client and server each use the
unauthenticated key shares to generate secrets which are used for
protection of the remaining handshake records. Specifically, they
generate:

       client_hs_master_secret = PRF(pre_master_secret,
                                     "client_hs_master_secret",
                                     session_hash)
                                     [0..47];

       server_hs_master_secret = PRF(pre_master_secret,
                                     "server_hs_master_secret",
                                     session_hash)
                                     [0..47];
...
The session_hash value is defined in {{the-session-hash}}.

These secret values are used to generate the record protection
keys used for the handshake, as described in {{key-calculation}}.
The client_hs_master_secret is used to protect traffic which is sent
...
of the session:
       ...
       server_master_secret = PRF(pre_master_secret,
                                  "server_master_secret",
                                  session_hash)
                                  [0..47];

       resumption_master_secret = PRF(pre_master_secret,
...
traffic sent by the server to the client. The resumption_master_secret
it stored in the session cache for resumption.

Once these computations have been completed, the pre_master_secret
is no longer necessary and should be deleted from memory.
